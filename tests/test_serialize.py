...
def test_operators_serialization_minimal(config_file, temp_dir):
    """Test serializing operators with save_arrays=False and regenerating from cache.
    ...
    assert (output_dir / ".regenerate_operators").exists(), "Marker file not created"
    ...
    # Deserialize the operators - this should regenerate from cache
    deserialized_operators = celeri.Operators.from_disk(output_dir)
...
def test_estimation_serialization_minimal(config_file, temp_dir):
    """Test serializing estimation with save_operators=False and regenerating from cache.
    ...
    assert (output_dir / "operators" / ".regenerate_operators").exists(), (
        "Marker file not created"
    )

    # Deserialize the estimation - operators should be regenerated from cache
    deserialized_estimation = Estimation.from_disk(output_dir)

    # Verify results match exactly - operators regenerated from same cache
