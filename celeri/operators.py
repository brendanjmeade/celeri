...
class Operators:
    ...
    """Moment tensors for each block (dimensionless, on unit sphere).

    Shape (n_blocks, 3, 3). The moment tensor characterizes
    the spatial distribution of a block for computing mean velocities.
...
def _store_block_moment_tensors(model: Model, operators: "_OperatorBuilder"):
    ...
    for block_idx in range(n_blocks):
        ...
        if polygon.vertices_xyz is None:
            raise ValueError(
        ...
        M, area = compute_moment_tensor_lambert(
            vertices_xyz,
            # We use the centroid instead of the interior point
            # because it often is a better center for the transformation
