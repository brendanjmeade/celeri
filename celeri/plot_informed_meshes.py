...
mesh = estimation.model.meshes[0]

mesh_lat = [estimation.model.meshes[idx].lat_centroid.mean() for idx in stds.iloc[stds < std_cutoff].index]
mesh_lon = [estimation.model.meshes[idx].lon_centroid.mean() for idx in stds.iloc[stds < std_cutoff].index]
low_std_mesh_idxs = stds.iloc[stds < std_cutoff].index
low_std_mesh_idxs
...
plt.scatter(np.array(mesh_lon) % 360, mesh_lat, marker="D", zorder=1000, color="red", s=100)
