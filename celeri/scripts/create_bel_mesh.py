...
def main():
    ...
    buffered_polygon = transform(
        transformer_to_lonlat.transform, buffered_polygon_local
    )
    ...
    boundary_spacing = (
        args.msh_element_length_scale * 1000 * 0.5
    )  # Half the element size in meters
    ...
    output_file = Path(
        f"{args.msh_file.stem}_{args.msh_buffer_length_scale}_{abs(depth)}_{args.msh_element_length_scale}.msh"
    )
    ...
    print(
        f"Successfully created horizontal mesh with {len(coords)} nodes at depth {depth} km"
    )
